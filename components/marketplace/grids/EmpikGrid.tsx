...
// ─────────────────────────────────────────────────────────────────────────────
// EMPIK — samodzielna siatka (pełna logika, brak współdzielenia z innymi rynkami).
// Edycja tego pliku NIE wpływa na Allegro / BRW / Kaufland.
// ─────────────────────────────────────────────────────────────────────────────
...
const SLUG = 'empik';
const editPath = (typesenseId: string) => `/products/${typesenseId}/add-to-empik`;
...
export default function EmpikGrid() {
  const def = getMarketplace(SLUG);
  const mpLabel = def?.name ?? SLUG;
  const accent = ACCENT_CLASSES[def?.accent ?? 'indigo'];
  ...
  const runSync = async () => {
    ...
    try {
      ...
      toast.success('Zaktualizowano z Empik');
      await loadFromDb();
