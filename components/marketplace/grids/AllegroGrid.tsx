...
// ─────────────────────────────────────────────────────────────────────────────
// ALLEGRO — samodzielna siatka (pełna logika, brak współdzielenia).
// Edycja tego pliku NIE wpływa na Empik / BRW / Kaufland.
// Sync iteruje po wszystkich aktywnych kontach Allegro.
// ─────────────────────────────────────────────────────────────────────────────
...
const SLUG = 'allegro';
const editPath = (typesenseId: string) => `/products/${typesenseId}/add-to-allegro`;
...
export default function AllegroGrid() {
  const def = getMarketplace(SLUG);
  const mpLabel = def?.name ?? SLUG;
  const accent = ACCENT_CLASSES[def?.accent ?? 'allegro'];
  ...
  useEffect(() => {
    ...
    if (filterGlobal === 'all') {
      ...
      const filtered = rows.filter(row => {
        const status = row['publication.status'] ;
       if( status == filterGlobal) sts.add(status+"--"+filterGlobal);
        return status == filterGlobal;
      });
  ...
  const loadFromDb = async () => {
    ...
    try {
      ...
      const newRows = (d?.items || []).map(toRow);
      setRows(newRows);
  ...
  const runSync = async () => {
    ...
    try {
      ...
      const accountIds: string[] = accs.filter((a: { is_active: number }) => a.is_active).map((a: { account_id: string }) => a.account_id);
      ...
      for (const accountId of accountIds) {
        ...
        do {
          ...
          for (let a = 0; a < 4; a++) {
            res = await fetch(`/api/marketplace/sync-live?slug=${SLUG}&offset=${offset}&limit=100&runStartedAt=${runStartedAt}&accountId=${encodeURIComponent(accountId)}`, { method: 'POST' })
          ...
          grand = Math.max(grand, (res!.total || 0) * accountIds.length);
      ...
      toast.success('Zaktualizowano z Allegro');
      await loadFromDb();
  ...
  const toolbarLeft = (
    <>
      <h1 className="text-base font-bold text-gray-900 mr-2">{mpLabel} <span className="text-gray-400 font-normal">({filteredRows.length})</span></h1>
      ...
      <div className="row flex">
        <button onClick={() => handleGlobalFilter('ACTIVE')}
          className={(filterGlobal == "ACTIVE" ? 'bg-green-500 ' : 'bg-slate-800 ') + `rounded-md rounded-r-none py-2 px-3 text-xs border border-transparent text-center text-white transition-all shadow-md hover:shadow-lg focus:bg-slate-700 focus:shadow-none active:bg-slate-700 hover:bg-slate-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none`}
          type="button">
          Aktywne
        </button>
        <button
          onClick={() => handleGlobalFilter('ACTIVATING')}
          className={(filterGlobal == "ACTIVATING" ? 'bg-green-500 ' : 'bg-slate-800 ') + `rounded-none text-xs py-2 px-3 border-l border-r border-slate-700 text-center text-white transition-all shadow-md hover:shadow-lg focus:bg-slate-700 focus:shadow-none active:bg-slate-700 hover:bg-slate-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none`}
          type="button">
          Draft
        </button>
        <button
          onClick={() => handleGlobalFilter('INACTIVE')}
          className={(filterGlobal == "INACTIVE" ? 'bg-green-500 ' : 'bg-slate-800 ') + `rounded-none text-xs py-2 px-3 border-l border-r border-slate-700 text-center text-white transition-all shadow-md hover:shadow-lg focus:bg-slate-700 focus:shadow-none active:bg-slate-700 hover:bg-slate-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none`}
          type="button">
          Nieaktywne
        </button>
        <button
          onClick={() => handleGlobalFilter('ENDED')}
          className={(filterGlobal == "ENDED" ? 'bg-green-500 ' : 'bg-slate-800 ') + `rounded-none text-xs py-2 px-3 border-l border-r border-slate-700 text-center text-white transition-all shadow-md hover:shadow-lg focus:bg-slate-700 focus:shadow-none active:bg-slate-700 hover:bg-slate-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none`}
          type="button">
          Zakończona
        </button>
        <button
          onClick={() => handleGlobalFilter('all')}
          className={(filterGlobal == "all" ? 'bg-green-500 ' : 'bg-slate-800 ') + `rounded-md rounded-l-none text-xs py-2 px-3 border border-transparent text-center text-white transition-all shadow-md hover:shadow-lg focus:bg-slate-700 focus:shadow-none active:bg-slate-700 hover:bg-slate-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none`}
          type="button">
          ALL 
        </button>
      </div>
