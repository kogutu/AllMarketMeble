...
// ─────────────────────────────────────────────────────────────────────────────
// KAUFLAND — samodzielna siatka (pełna logika, brak współdzielenia).
// Edycja tego pliku NIE wpływa na Empik / Allegro / BRW. (Publikacja = etap 2.)
// ─────────────────────────────────────────────────────────────────────────────
...
const SLUG = 'kaufland';
const editPath = (typesenseId: string) => `/products/${typesenseId}/add-to-kaufland`;
...
export default function KauflandGrid() {
  const def = getMarketplace(SLUG);
  const mpLabel = def?.name ?? SLUG;
  const accent = ACCENT_CLASSES[def?.accent ?? 'green'];
  ...
  const runSync = async () => {
    ...
    try {
      ...
      toast.success('Zaktualizowano z Kaufland');
      await loadFromDb();
